...
int32_t CUSTOM_ISPU_MOTION_SENSOR_Get_DRDY_Status(uint32_t Instance, uint32_t Function, uint8_t *Status)
{
  ...
  switch (Instance)
  {
    ...
    case CUSTOM_ISM330IS_0:
      ...
      {
        if (ISM330IS_ACC_Get_DRDY_Status((ISM330IS_Object_t *)IspuMotionCompObj[Instance], Status) != BSP_ERROR_NONE)
        {
          ret = BSP_ERROR_COMPONENT_FAILURE;
        }
        else
      ...
      {
        if (ISM330IS_GYRO_Get_DRDY_Status((ISM330IS_Object_t *)IspuMotionCompObj[Instance], Status) != BSP_ERROR_NONE)
        {
          ret = BSP_ERROR_COMPONENT_FAILURE;
        }
        else
