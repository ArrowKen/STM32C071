...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ISM330IS_ACC      0x01U
#define ISM330IS_GYRO     0x02U
#define ISM330IS_FUNC     0x04U
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
uint8_t tx_buf[1] = {0x9F};   // JEDEC ID 指令
uint8_t rx_buf[3] = {0};      // 回傳的 3 byte 資料
/* USER CODE END PV */
...
{
    uint8_t who_am_i = 0;
    HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI2_NSS_Pin, GPIO_PIN_RESET); //選擇SPI mode
    // 初始化感測器
    if (CUSTOM_ISPU_MOTION_SENSOR_Init(0, ISM330IS_FUNC) != BSP_ERROR_NONE)
    {
    ...
    // 讀取 WHO_AM_I
    if (CUSTOM_ISPU_MOTION_SENSOR_ReadID(0, &who_am_i) == BSP_ERROR_NONE)
    {
        printf("WHO_AM_I = 0x%02X\r\n", who_am_i);

        if (who_am_i == 0x6B)
            printf("ISM330IS Detected!\r\n");
        else
...
void Read_Flash_JEDEC_ID(void)
{
    ...
    HAL_SPI_Transmit(&hspi1, tx_buf, 1, HAL_MAX_DELAY);  // 傳送 0x9F 指令
    HAL_SPI_Receive(&hspi1, rx_buf, 3, HAL_MAX_DELAY);   // 接收 3 bytes 回應

    HAL_GPIO_WritePin(SPI1_NSS_GPIO_Port, SPI1_NSS_Pin, GPIO_PIN_SET); // 釋放 CS
